...
thumbs.on("click",".owl-item",function(e){e.preventDefault();var number=$(this).index();bigimage.data("owl.carousel").to(number,300,true);});$('body').append("");$('body').append("<div class='switch-box'><label id='switch' class='switch'><input type='checkbox' onchange='toggleTheme()' id='slider'><span class='slider round'></span></label></div>");})(jQuery);function setTheme(themeName){localStorage.setItem('ezio_rtl_theme',themeName);document.documentElement.className=themeName;}
function toggleTheme(){if(localStorage.getItem('ezio_rtl_theme')==='theme-dark'){setTheme('theme-light'); location.reload();
}else{setTheme('theme-dark');location.reload();}}
(function(){if(localStorage.getItem('ezio_rtl_theme')==='theme-dark'){setTheme('theme-dark');document.getElementById('slider').checked=false;}else{setTheme('theme-light');document.getElementById('slider').checked=true;}})();

{
    class SliderClip {
      constructor(el) {
        ...
        this.timeTrans = 2000; //transition time in milliseconds
        this.IndexElements = [];
